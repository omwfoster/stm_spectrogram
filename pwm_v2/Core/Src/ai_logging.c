...
void ai_logging_init_send(ai_logging_device_t *device,
		uint32_t (*send_fptr)(uint8_t *data_ptr, uint32_t data_size),
		uint8_t *send_buffer, uint32_t send_buffer_max_size) {
	device->send_fptr = send_fptr;
	device->send_buffer = send_buffer;
	device->send_buffer_max_size = send_buffer_max_size;
}

void AI_LOGGING_DLL ai_logging_init_send_ctx(ai_logging_device_t *device,
		uint32_t (*send_ctx_fptr)(void *ctx, uint8_t *data_ptr,
				uint32_t data_size), void *ctx, uint8_t *send_buffer,
...
void ai_logging_init_receive(ai_logging_device_t *device,
		uint32_t (*receive_fptr)(uint8_t *data_ptr, uint32_t data_size),
		uint32_t receive_fptr_size_to_read, uint8_t *receive_buffer,
		uint32_t receive_buffer_max_size) {
...
void AI_LOGGING_DLL ai_logging_init_receive_ctx(ai_logging_device_t *device,
		uint32_t (*receive_ctx_fptr)(void *ctx, uint8_t *data_ptr,
...
void ai_logging_init_raw_data_buffer(ai_logging_device_t *device,
		uint8_t *user_raw_buffer) {
	device->user_raw_buffer = user_raw_buffer;
}

void ai_logging_send_data(ai_logging_device_t *device, ai_logging_buffer data,
...
void AI_LOGGING_DLL ai_logging_send_message(ai_logging_device_t *device,
...
void AI_LOGGING_DLL ai_logging_send_message_with_data(
		ai_logging_device_t *device, ai_logging_message message,
		ai_logging_buffer data, ai_logging_tensor data_info) {
	uint32_t message_data_size = 0;
	if (message.size != 0) { // If we have a message
		message_data_size = AI_LOGGING_PAYLOAD_COMMAND_FIXED_SIZE
				+ message.size;
	}
	...
	if (data_info.shape->n_shape != 0) { // If shape is not 0
		...
		payload_shape_size = AI_LOGGING_PAYLOAD_SHAPE_FIXED_SIZE
				+ payload_shape_data_size;
	}
	...
	uint32_t total_payload_size = message_data_size + payload_shape_size
			+ payload_data_size;
	...
	add_data_to_buffer(device, temp, sizeof(uint8_t));
	add_data_to_buffer(device, (uint8_t*) &total_payload_size,
			sizeof(uint32_t));
	...
	if (message.size != 0) {
		...
		if (message.size + device->send_buffer_position
				> device->send_buffer_max_size) {
			while (1)
				; // Error internal buffer must be at least 255 + 7 byte (Max uint8 + header size)
	...
	if (data_info.shape->n_shape != 0) { // If shape is not 0
		temp[0] = AI_SHAPE;
		add_data_to_buffer(device, temp, sizeof(uint8_t));
		add_data_to_buffer(device, (uint8_t*) &payload_shape_data_size,
				sizeof(payload_shape_data_size));
		int i = 0;
		for (i = 0; i < data_info.shape->n_shape; i++) {
			add_data_to_buffer(device, (uint8_t*) &(data_info.shape->shapes[i]),
					sizeof(uint16_t));
		}
	}

	if (data.size != 0) { // If data_size is not 0
		add_data_to_buffer(device, (uint8_t*) &(data_info.data_type),
				sizeof(uint8_t));
		add_data_to_buffer(device, (uint8_t*) &(data.size), sizeof(uint32_t));

		if (data.size + device->send_buffer_position
				> device->send_buffer_max_size) // Decide weather to perform copy to buffer or not
				{
			...
			// }
			_ai_logging_call_send_fptr(device, device->send_buffer,
					device->send_buffer_position);
...
int _ai_logging_prepare_raw_header_in_buffer(uint8_t *buffer, uint8_t data_type,
...
void ai_logging_send_header_for_raw_data(ai_logging_device_t *device,
		uint8_t data_type, uint32_t total_data_length) {
	/*
	...
	add_data_to_buffer(device, (uint8_t*) &total_data_length,
			sizeof(total_data_length));
...
void ai_logging_send_raw_data(ai_logging_device_t *device, uint8_t *buffer,
...
void ai_logging_data_received_callback(ai_logging_device_t *device,
		uint8_t *data_ptr, uint32_t data_size) {
	if (device->copy_next_byte_in_user_buffer > 0) {
		if (data_size < (uint32_t) device->copy_next_byte_in_user_buffer) {
			memcpy(device->user_raw_buffer + device->user_buffer_position,
					data_ptr, data_size);
			device->copy_next_byte_in_user_buffer -= data_size;
			device->user_buffer_position += data_size;
		} else {
			memcpy(device->user_raw_buffer + device->user_buffer_position,
					data_ptr, device->copy_next_byte_in_user_buffer);
			uint32_t bytes_left_to_copy = data_size
					- device->copy_next_byte_in_user_buffer;

			if (device->receive_buffer_position + bytes_left_to_copy
					> device->receive_buffer_max_size) {
				device->receive_buffer_read_from = 0; // In this case we reset position because old data will be overriden
				device->receive_buffer_position = 0;
			}
			memcpy(device->receive_buffer + device->receive_buffer_position,
					data_ptr + device->copy_next_byte_in_user_buffer,
					bytes_left_to_copy);
			device->receive_buffer_position += bytes_left_to_copy;
			device->user_buffer_position +=
					device->copy_next_byte_in_user_buffer;
			device->copy_next_byte_in_user_buffer = 0;
		}
	} else {
		if (device->receive_buffer_position + data_size
				> device->receive_buffer_max_size) {
			device->receive_buffer_read_from = 0; // In this case we reset position because old data will be overriden
			device->receive_buffer_position = 0;
		}
...
int ai_logging_wait_for_data(ai_logging_device_t *device,
	...
	while (ret < 0) {
		...
		if (device->receive_buffer_position + device->receive_fptr_size_to_read
				> device->receive_buffer_max_size) {
			device->receive_buffer_read_from = 0; // In this case we reset position because old data will be overriden
		...
		if (device->receive_fptr != NULL) {
			uint32_t transferred = device->receive_fptr(
					device->receive_buffer + device->receive_buffer_position,
					device->receive_fptr_size_to_read);
			device->receive_buffer_position += transferred;
		} else if (device->receive_ctx_fptr != NULL) {
			uint32_t transferred = device->receive_ctx_fptr(
					device->receive_function_ctx,
					device->receive_buffer + device->receive_buffer_position,
					device->receive_fptr_size_to_read);
			device->receive_buffer_position += transferred;
		} else {
...
int ai_logging_check_for_received_packet(ai_logging_device_t *device,
		ai_logging_packet_t *packet) {
	...
	while (device->receive_buffer_read_from <= device->receive_buffer_position) {
		...
		//   device->receive_buffer_read_from = 0;
		// }
		if (device->receive_buffer_read_from
				== device->receive_buffer_position) {
			return AI_NOT_ENOUGH_DATA_READ;
		}

		int32_t start_byte_index = ai_logging_find_start_byte(device,
				device->receive_buffer_read_from);
		...
		if (ai_logging_is_there_packet_type_v1(device, start_byte_index + 1)) {
			int ret = ai_logging_decode_v1_packet(device, packet,
					start_byte_index);
		...
		} else if (device->user_raw_buffer != NULL
				&& ai_logging_is_there_packet_type_v2(device,
						start_byte_index + 1)) {
			int ret = ai_logging_decode_v2_packet(device, packet,
					start_byte_index);
...
int ai_logging_decode_v1_packet(ai_logging_device_t *device,
		ai_logging_packet_t *packet, int32_t start_byte_index) {
	uint32_t current_index = start_byte_index + 1 + 1; //Placing after Start byte + packet type byte
	uint32_t packet_length = 0;
	current_index = read_uint32_from_buffer(device->receive_buffer,
			current_index, &packet_length);
	if (packet_length == 0) {
		device->receive_buffer_read_from = current_index;
		return AI_PACKET_ERROR; // No packet of size 0 : strange
	}
	if (device->receive_buffer_position
			< start_byte_index + packet_length + 1 + 4 + 1 + 1) {
		// StartByte(1) + PacketType(1) + PacketSize(4) + [PAYLOAD] + EndByte(1)
		// We haven't read enough data: we wait for next round
		return AI_NOT_ENOUGH_DATA_READ;
	}

	uint32_t end_byte_index = (start_byte_index + 1 + 4 + packet_length + 1)
			% device->receive_buffer_max_size;
	...
	for (uint32_t i = payloads_start_index;
			i < payloads_start_index + packet_length; i++) {
		if (device->receive_buffer[i] == AI_COMMAND) {
			// Reminder: PayloadType(A byte) + PayloadLength(1 byte) + (PayloadLength)
			uint8_t command_length = 0;
			read_uint8_from_buffer(device->receive_buffer, i + 1,
					&command_length);
			if (command_length == 0) {
			...
			read_uint16_from_buffer(device->receive_buffer, i + 1,
					&shape_length);
			if (shape_length == 0) {
				device->receive_buffer_read_from = end_byte_index;
			...
			for (int k = 0; k < nb_shape; k++) {
				...
				read_uint16_from_buffer(device->receive_buffer,
						i + 1 + 2 + k * 2, &shape_value);
			...
			data_length = 0;
			read_uint32_from_buffer(device->receive_buffer, i + 1,
					&data_length);
			if (data_length == 0) {
				device->receive_buffer_read_from = end_byte_index;
			...
			packet->payload_type =
					(ai_logging_payload_type) device->receive_buffer[i];
...
int ai_logging_decode_v2_packet(ai_logging_device_t *device,
		ai_logging_packet_t *packet, int32_t start_byte_index) {
	/*
	...
	uint32_t header_end_position = (uint32_t) start_byte_index + 1 + 1 + 4 + 1
			+ 4; // Skipping SB(1),PType(1),PLength(4),PayloadType(1),PayloadLength(4)
...
//Assuming we have the complete header
	...
	current_index = read_uint8_from_buffer(device->receive_buffer,
			current_index, &payload_type);
	if (payload_type == 0) {
	...
	if (device->copy_next_byte_in_user_buffer == 0) {
		...
		memcpy(device->user_raw_buffer, device->receive_buffer + current_index,
				byte_to_copy); // Moving byte from internal to user buffer
...
void ai_logging_prepare_next_packet(ai_logging_device_t *device) {
	...
	if (device->receive_buffer_position > last_packet_end_index) {
		size_to_copy = device->receive_buffer_position - last_packet_end_index
				- 1;
	}

//memcpy(device->receive_buffer, device->receive_buffer+last_packet_end_index + 1, size_to_copy);
	memmove(device->receive_buffer,
			device->receive_buffer + last_packet_end_index + 1, size_to_copy);
	device->receive_buffer_read_from = 0;
	device->receive_buffer_position = size_to_copy;
}

void ai_logging_send_packet(ai_logging_device_t *device,
		ai_logging_packet_t *packet) {
	uint32_t message_data_size = 0;
	...
	if (packet->payload_size != 0) { // If data_size is not 0
		payload_data_size = AI_LOGGING_PAYLOAD_DATA_FIXED_SIZE
				+ packet->payload_size;
	}
	uint32_t total_payload_size = message_data_size + payload_shape_size
			+ payload_data_size;
	...
	if (packet->message_size != 0) {
		...
		add_data_to_buffer(device, (uint8_t*) &(packet->message_size),
				sizeof(uint8_t)); // Size of command

		if (packet->message_size + device->send_buffer_position
				> device->send_buffer_max_size) {
			while (1)
				; // Error internal buffer must be at least 255 + 7 byte (Max uint8 + header size)
			//Improvement we can manage this case
	...
	if (packet->shape.n_shape != 0) { // If shape is not 0
		...
		for (i = 0; i < packet->shape.n_shape; i++) {
			add_data_to_buffer(device, (uint8_t*) &(packet->shape.shapes[i]),
					sizeof(uint16_t));
		}
	}
	...
	if (packet->timestamp >= 0) {
		temp[0] = AI_TIMESTAMP;
		add_data_to_buffer(device, temp, sizeof(uint8_t));
		add_data_to_buffer(device, (uint8_t*) &(packet->timestamp),
				sizeof(int32_t));
	}

	if (packet->payload_size != 0) { // If data_size is not 0
		add_data_to_buffer(device, (uint8_t*) &(packet->payload_type),
				sizeof(uint8_t));
		add_data_to_buffer(device, (uint8_t*) &(packet->payload_size),
				sizeof(uint32_t));

		if (packet->payload_size + device->send_buffer_position
				> device->send_buffer_max_size) // Decide weather to perform copy to buffer or not
				{
			_ai_logging_call_send_fptr(device, device->send_buffer,
					device->send_buffer_position);
			...
			device->send_buffer_position = 0;
			_ai_logging_call_send_fptr(device, packet->payload,
					packet->payload_size);
...
uint8_t ai_logging_is_there_packet_type_v1(ai_logging_device_t *device,
		uint32_t at) {
...
uint8_t ai_logging_is_there_packet_type_v2(ai_logging_device_t *device,
		uint32_t at) {
...
// Global Helpers
uint32_t read_uint32_from_buffer(uint8_t *buffer, uint32_t shift,
		uint32_t *data_out) {
	memcpy(data_out, buffer + shift, 4);
	return shift + 4;
}

int32_t read_int32_from_buffer(uint8_t *buffer, uint32_t shift,
		int32_t *data_out) {
	memcpy(data_out, buffer + shift, 4);
	return shift + 4;
}

uint32_t read_uint16_from_buffer(uint8_t *buffer, uint32_t shift,
		uint16_t *data_out) {
	memcpy(data_out, buffer + shift, 2);
	return shift + 2;
}

uint32_t read_uint8_from_buffer(uint8_t *buffer, uint32_t shift,
		uint8_t *data_out) {
...
void _ai_logging_call_send_fptr(ai_logging_device_t *device, uint8_t *data_ptr,
		uint32_t data_size) {
	if (device->send_function_ctx == NULL) {
		...
		if (device->send_ctx_fptr != NULL) {
			device->send_ctx_fptr(device->send_function_ctx, data_ptr,
					data_size);
		} else {
			_ai_logging_fire_error(device);
		}
...
void add_data_to_buffer(ai_logging_device_t *device, uint8_t *data_ptr,
		uint16_t size) {
	if (device->send_buffer_position + size > device->send_buffer_max_size) {
		_ai_logging_fire_error(device);
	} else {
		memcpy(device->send_buffer + device->send_buffer_position, data_ptr,
				size);
...
void ai_logging_create_shape_2d(ai_logging_shape *shape, uint16_t dim_x,
		uint16_t dim_y) {
	shape->n_shape = 2;
	shape->shapes[0] = dim_x;
	shape->shapes[1] = dim_y;
}
void ai_logging_create_shape_3d(ai_logging_shape *shape, uint16_t dim_x,
		uint16_t dim_y, uint16_t dim_z) {
	shape->n_shape = 3;
	shape->shapes[0] = dim_x;
	shape->shapes[1] = dim_y;
